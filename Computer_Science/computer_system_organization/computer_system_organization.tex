...
As MSB is also flipped, MSB will represent the sign as in "Sign Magnitude Representation".\\
\\
Ex:\\
...
Range: $-(2^{n-1} - 1) \rightarrow +(2^{n-1} - 1)$
\subsubsection*{c - Two's Complement Representation}
Here also, we display positive numbers using the previous "Sign Magnitude Representation" method, and flip the digits, and add 1 to make it negative.\\
As MSB is also flipped, MSB will represent the sign as in "Sign Magnitude Representation".\\
\\
Ex:\\
$+4_{10} = 0100_2 \rightarrow -4_{10} = 1011_2 + 1_2 = 1100_2 $\\
\\
Advantages of two's complement representation:
\begin{itemize}
  \item Unique representation of zero.
  \item Subtraction can be done using addition.
  \item Leads to substantial saving in circuitry.
\end{itemize}
Almost all computers today use 2’s complement representation for storing negative numbers.\\
Range: $-2^{n-1} \rightarrow 2^{n-1} - 1$
\subsubsection*{Converting Signed Binary Numbers to Decimal}
For signed integer representation, the MSB only represents the sign. \\The remaining bits represent the number.\\
Converting the remaining bits to decimal as usual, and assigning the sign given by MSB, will result in the decimal number.\\
\\
Ex:\\
$1011_2 \rightarrow 011_2 = 3_{10}$, and $MSB = 1$ is (-), therefore, $-3$.\\ 
\\
For one's complement and two's complement, all numbers represent a value, but the MSB also carries the sign.\\
Therefore, we use the normal methodology to convert to decimal, but we should be concerned about the sign of MSB,  when assigning the decimal value.\\
\\
Ex:\\
$1011_2$\\
$-2^3 \times 1 + 2^2 \times 0 + 2^1 \times 1 + 2^0 \times 1 = -8 + 2 + 1 = -5$\\
\\
$0101_2$\\
$+2^3 \times 0 + 2^2 \times 1 + 2^1 \times 0 + 2^0 \times 1 = +5$
\newpage
\subsection*{Decimal multiplication by 2 of known binary numbers}
Multiplication is done by left shifting by 1 digit, and adding a zero to the right last digit per one power of 2.\\
Left shifting a binary number moves every bit to a higher place value, doubling its contribution because each position represents a power of 2. The empty lowest position becomes 0, so each shift left multiplies the number by 2.\\
\\
Ex:\\
$19=00010011_2$\\
$38=00100110_2$
\subsection*{Decimal division by 2 of known binary numbers}
Division is done by right shifting by 1 digit, and adding a zero to the left first digit per one power of 2.\\
Right shifting a binary number moves every bit to a lower place value, halving its contribution because each position represents a power of 2. The lowest bit is dropped, so each shift right divides the number by 2 (discarding any remainder).\\
\\
Ex:\\
$38=00100110_2$\\
$19=00010011_2$
